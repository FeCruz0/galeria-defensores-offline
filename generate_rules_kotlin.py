...
# Full text of rules (including the part I already wrote, just in case, or append)
# Actually, I'll write the REST of the text here and append it, then read the whole file.
remaining_text = """MANOBRAS ESPECIAIS
...
# Append remaining text to rules_raw.txt
with open("rules_raw.txt", "a", encoding="utf-8") as f:
    f.write(remaining_text)

# Read full file
with open("rules_raw.txt", "r", encoding="utf-8") as f:
    full_text = f.read()
...
replacements = {
    ...
    r"\\bPRT\\b": "Perto",
    r"\\bLNG\\b": "Longe",
    r"\\bFDA\\b": "Fora do Alcance",
    r"\\bACT\\b": "Ação",
    r"\\bMOV\\b": "Movimento",
    r"\\bINI\\b": "Iniciativa",
    r"\\bCNA\\b": "Cena",
    r"\\bSTN\\b": "Status Negativo",
    r"\\bEQP\\b": "Equipamento",
    r"\\bPDR\\b": "Poder",
    r"\\bPRC\\b": "Perícia",
    r"\\bATV\\b": "Poder Ativo",
    r"\\bPSV\\b": "Poder Passivo",
    r"\\bTEC\\b": "Técnica"
}

# Apply replacements with regex for word boundaries
for abbr, full_word in replacements.items():
    full_text = re.sub(abbr, full_word, full_text)

# Apply special characters escaping for Kotlin string literal
# Replace double quotes with escaped double quotes
full_text = full_text.replace('"', '\\"')

# Simple parser to extract ItemDefinition format
sections = []
...
for line in lines:
    ...
    # Heuristic for header: Upper case and no colons/periods at end preferably, or known headers
    # Some headers might have spaces inside.
    # The text structure is consistently HEADER then content.
    # Headers in this list are like "ACELERAÇÃO", "ADAPTADOR".
    # Checking if line is uppercase. Some lines inside body might be uppercase though? Unlikely in this text.
    # Wait, "1PT", "2PT" etc are uppercase. So I should ignore lines starting with digit.
    if line.isupper() and not line[0].isdigit() and len(line) > 2:
        if current_section:
...
kotlin_code = ""
kotlin_code += '    private fun getAdvantages(): MutableList<ItemDefinition> {\n'
kotlin_code += '        val list = mutableListOf<ItemDefinition>()\n\n'

for name, body in sections:
    # Determine cost from body
    # Heuristic: Find "1PT", "2PT", "3PT".
    # If "1PT" and "3PT" present -> "1-3"
    # If "1PT" -> "1"
    # "ATAQUE ESPECIAL" mentions 5PT -> "1-5"
    cost = "1-3" 
    if "1PT" in body:
        if "5PT" in body or "Custo básico 1pt" in body:
             cost = "1-5"
        elif "3PT" in body:
             cost = "1-3"
        elif "2PT" in body:
             ...
             cost = "1"
             # Check for "Varia" or other costs? "MÁCULA" cost = "Varia"
             if "Varia" in body or "custo em PT" in body:
                 cost = "Varia"
    
    # Special cases
    if name == "EQUIPAMENTO ESPECIAL": cost = "1" # Body says 1PT. Sub-items have costs. But base adv is 1.
    ...
    # Convert newlines to \n literal for kotlin string
    body_escaped = body.replace("\n", "\\n")
    
    kotlin_code += f'        list.add(ItemDefinition(name = "{name}", cost = "{cost}", description = "{body_escaped}"))\n'
